...
{
    public record EditScheduleActivityCommand(Guid Id, Guid SessionId, EditScheduleActivityDto EditDto) : ICommand<EditScheduleActivityResult>;
    ...
    public class EditScheduleActivityHandler : ICommandHandler<EditScheduleActivityCommand, EditScheduleActivityResult>
    {
        ...
        public EditScheduleActivityHandler(SchedulingDbContext context, IPublishEndpoint publishEndpoint)
        {
            _context = context;
            _publishEndpoint = publishEndpoint;
        }

        public async Task<EditScheduleActivityResult> Handle(EditScheduleActivityCommand request, CancellationToken cancellationToken)
        {
            var activity = await _context.ScheduleActivities
                .FirstOrDefaultAsync(a => (a.EntertainmentActivityId == request.Id ||
                     ...
                     && a.SessionId == request.SessionId && a.SessionId == request.SessionId, cancellationToken);
