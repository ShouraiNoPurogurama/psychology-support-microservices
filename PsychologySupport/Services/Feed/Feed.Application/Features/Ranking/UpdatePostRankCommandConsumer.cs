    ...
    : IConsumer<UpdatePostRankCommand>
{
    ...
    public async Task Consume(ConsumeContext<UpdatePostRankCommand> context)
    {
        ...
        var rank = await rankingService.GetPostRankAsync(postId);
        if (rank is null)
        {
            ...
            rank = await rankingService.GetPostRankAsync(postId);
        ...
        var updated = rank with { Score = score, UpdatedAt = DateTimeOffset.UtcNow };

        await rankingService.UpdatePostRankAsync(postId, updated);
