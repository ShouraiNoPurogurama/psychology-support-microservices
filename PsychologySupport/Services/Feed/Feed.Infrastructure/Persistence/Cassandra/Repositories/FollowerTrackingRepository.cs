...
public sealed class FollowerTrackingRepository : IFollowerTrackingRepository
{
    ...
    public async Task<bool> AddIfNotExistsAsync(Follower entity, CancellationToken ct)
    {
        ...
        var cql = @"INSERT INTO followers_by_alias (follower_alias_id, alias_id, since)
                    VALUES (?, ?, ?) IF NOT EXISTS";
        var ps = await _session.PrepareAsync(cql).ConfigureAwait(false);
        var stmt = ps.Bind(rowData.FollowerAliasId, rowData.AliasId, rowData.Since).SetIdempotence(true);
    ...
    public async Task<bool> RemoveAsync(Guid followerAliasId, Guid aliasId, CancellationToken ct)
    {
        var cql = @"DELETE FROM followers_by_alias WHERE follower_alias_id = ? AND alias_id = ?";
        var ps = await _session.PrepareAsync(cql).ConfigureAwait(false);
        var stmt = ps.Bind(followerAliasId, aliasId).SetIdempotence(true);
        await _session.ExecuteAsync(stmt).ConfigureAwait(false);
        return true;
    }

    public async Task<bool> ExistsAsync(Guid followerAliasId, Guid aliasId, CancellationToken ct)
    {
        var cql = @"SELECT follower_alias_id FROM followers_by_alias WHERE follower_alias_id = ? AND alias_id = ? LIMIT 1";
        var ps = await _session.PrepareAsync(cql).ConfigureAwait(false);
        var stmt = ps.Bind(followerAliasId, aliasId).SetIdempotence(true);
    ...
    public async Task<IReadOnlyList<Follower>> GetAllFollowersAsync(Guid followerAliasId, CancellationToken ct)
    {
        var cql = @"SELECT follower_alias_id, alias_id, since FROM followers_by_alias WHERE follower_alias_id = ?";
        var ps = await _session.PrepareAsync(cql).ConfigureAwait(false);
        var stmt = ps.Bind(followerAliasId).SetIdempotence(true);
        var rs = await _session.ExecuteAsync(stmt).ConfigureAwait(false);
