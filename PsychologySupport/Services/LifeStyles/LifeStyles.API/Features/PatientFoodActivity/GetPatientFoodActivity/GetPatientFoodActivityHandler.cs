...
namespace LifeStyles.API.Features.PatientFoodActivity.GetPatientFoodActivity
{
    ...
    {
        public async Task<GetPatientFoodActivityResult> Handle(GetPatientFoodActivityQuery request, CancellationToken cancellationToken)
        {
            var activities = await context.PatientFoodActivities
                .Where(pfa => pfa.PatientProfileId == request.PatientProfileId)
