...
public sealed class CurrentActorAccessor : ICurrentActorAccessor
{
    ...
    public bool TryGetPatientId(out Guid patientId)
    {
        patientId = Guid.Empty;

        var aliasIdStr = _http.HttpContext?.User.FindFirstValue("patientId");

        if (!Guid.TryParse(aliasIdStr, out var id) || id == Guid.Empty)
        {
            _logger.LogWarning("Missing or invalid patientId claim.");
            return false;
        }
    ...
    public Guid GetRequiredPatientId()
    {
        if (TryGetPatientId(out var id)) return id;
        throw new UnauthorizedException("Yêu cầu không hợp lệ.", "CLAIMS_MISSING");
    }
}