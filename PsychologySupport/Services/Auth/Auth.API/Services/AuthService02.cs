...
using Profile.API.Protos; // Add for PatientProfileService gRPC
using Notification.API.Protos; // Add for NotificationService gRPC

namespace Auth.API.Services;

public class AuthService02(
...
) : IAuthService02
{
    ...
    private async Task<(bool IsSuccess, string? ErrorMessage)> CreateUserProfileAsync(User user)
    {
        try
        {
            var contactInfo = new Profile.API.Protos.ContactInfo
            {
                Address = "None",
                Email = user.Email,
